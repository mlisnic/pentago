...
(define (all-possible player b)
  (local
    {(: checkloc : Loc -> (Listof Move))
     (define (checkloc loc)
       (match loc
         [(Loc row col)
          (if (and (eq? row 5) (eq? col 5)) '()
              (if (eq? (board-ref b loc) 'none)
                  (append (all-twists loc)
                          (checkloc (next-loc loc)))
                  (checkloc (next-loc loc))))]))}
    (checkloc (Loc 0 0))))

(: direct-win-move? : Player Board Move -> Boolean)
(define (direct-win-move? player b m)
  (match m
    [(Move loc q dir)
     (local
       {(define placed
          (place-marble (Game b player 'place 'aa 'bb) player loc))
        (define twisted
          (twist-quadrant placed q dir))}
       (cond
         [(eq? player 'black)
          (and (five-black? (Game-board placed))
              (five-black? (Game-board twisted)))]
         [else
          (and (five-white? (Game-board placed))
              (five-white? (Game-board twisted)))]))]))
         

(: direct-win : Player Board -> (Optional (U Loc Move)))
;; finds a move that will result in immediate victory
(define (direct-win player b)
  (local
    {(define options
       (all-possible player b))
     (: lp : (Listof Move) -> (Optional (U Loc Move)))
     (define (lp ms)
       (match ms
         ['() 'none]
         [(cons hd tl)
          (if (direct-win-move? player b hd)
              (Some hd)
              (lp tl))]))}
    (lp options)))

(: best-move : Player Board (Listof Move) Heuristic -> (Optional Move))
(define (best-move player b ms heur)
  (local
    {(define winning (direct-win player b))
     (: lp : (Listof Move) Move -> Move)
     (define (lp ms m)
       (match m
         [(Move loc q dir)
          (local
            {(define placed
             ...
             (define twisted
               (twist-quadrant placed q dir))
             (define max-so-far
               (heur player (Game-board twisted)))}
            (match ms
              ['() m]
              [(cons hd tl)
               (if (> (heur player (Game-board (twist-quadrant
                                   (place-marble (Game b player 'place 'aa 'bb)
                                                 player
                                                 loc)
                                   q dir)))
                      max-so-far)
...
;; makes a bot mind function given player and heuristic function
(define (make-mind player heur)
  (λ ([g : Game])
    (match g
      [(Game b n-p n-a p1 p2)
       (best-move player b (all-possible player b) heur)])))
